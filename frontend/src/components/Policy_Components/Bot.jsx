...
const Bot = () => {
  ...
  const ai_chat_bot = (policy_name,query)=>{
      axios
      .get('http://127.0.0.1:7000/bajaj/ai_bot', {
        headers: { 
  ...
  const bot_speak = (question, rate = 150) =>{
      axios
        .get('http://127.0.0.1:8000/speak', {
          headers: {
            prompt: question,
            rate: rate,
          },
  ...
  const bot_listen = () =>{
    axios
    .get('http://127.0.0.1:8000/listen')
    .then((response) => {
